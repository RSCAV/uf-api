...
> Auto-generated from `discovery/breadth-2026.json` + `discovery/apix-surface.json` (the live multi-agent discovery sweep). This is the wide map; the curated, badge-annotated subset lives in [ENDPOINTS.md](./ENDPOINTS.md), and the working clients are in [`src/`](./src).

Liveness: ✅ live · 🔒 forbidden/auth · ❌ dead · ⚠️ unknown. Relevance to a degree planner: 🎯 core · ➕ enrichment · 🏛 heritage · · none.


---


**Totals:** 7 surfaces mapped, 59 live endpoints (plus the apix gateway above).


## ONE.UF `apix` gateway (from `discover.ts`)

Automated bundle-grep found **10 apix paths** across **7 eager public services**, plus **15 login-gated features** from the public service catalog.
...
**Login-gated features (off-limits — student/staff SSO):** `/soc` (student), `/myschedule` (student), `/myschedule` (student), `/transcript` (student), `/degreeaudit` (student), `/about/transcriptorder` (student), `/applicants` (applicants), `/applicants` (applicants), `/news` (applicants), `/staff` (staff), `/staff` (staff), `/timeapp` (staff)
...
|---|---|---|---|---|---|
| Ribbit getcourse.rjs (course detail + prereqs + gen-ed flags… | GET | ✅ | – | 🎯 core | Primary single-course fetch: description, credits, grading scheme, prerequisite EDGES (as … |
| Ribbit getcourseeco.rjs (course->program reverse index) | GET | ✅ | – | 🎯 core | NEW DISCOVERY — builds the course->degree reverse graph: given a course, returns every deg… |
| Ribbit getprogram.rjs (program info envelope) | GET | ✅ | – | 🏛 heritage | Route exists but returns no data on UF's install — do NOT rely on it. Get degree requireme… |
| Ribbit getcourselist.rjs / getplan.rjs / getsection.rjs / ge… | GET | ❌ | – | · none | None — do not use. Section/term data comes from the ONE.UF Schedule-of-Courses API, not fr… |
| Course catalog sitemap (master crawl seed) | GET | ✅ | – | 🎯 core | The canonical enumeration of every catalog page. Partition into program vs subject URLs to… |
| Subject course bulk page (all courses for a subject) | GET | ✅ | – | 🎯 core | PRIMARY bulk ingest for descriptions + prereq edges + gen-ed flags. 138 requests cover the… |
| Program/degree requirement page (plangrid + courselist) | GET | ✅ | – | 🎯 core | PRIMARY degree-requirement source. sc_courselist = the structured requirement DAG (groups,… |
| Catalog search results (/search/?P=) | GET | ✅ | – | ➕ enrich | This is the href target behind every prereq <a> link (bubblelink code -> /search/?P=CODE).… |
| CourseLeaf JS bundle (API discovery source) | GET | ✅ | – | 🏛 heritage | Reference only — documents how the catalog builds Ribbit calls (the SPACE-form code requir… |
...
| Endpoint | M | Live | Auth | Rel | Use |
|---|---|---|---|---|---|
| graphql.__schema introspection | POST | ✅ | – | ➕ enrich | Schema discovery — lets the SDK auto-validate query shape and detect when RMP changes fiel… |
| newSearch.schools (find UF school ID) | POST | ✅ | – | 🎯 core | One-time lookup to hardcode UF school id (U2Nob29sLTExMDA=). All teacher searches scope to… |
| newSearch.teachers (search UF prof by name) | POST | ✅ | – | 🎯 core | Primary lookup: map an instructor name (from SOC sections) to an RMP Teacher node id withi… |
| node(id) on Teacher (full detail) | POST | ✅ | – | 🎯 core | The instructor-quality record: overall quality/difficulty/would-take-again, star distribut… |
| Teacher.ratings (paginated reviews) | POST | ✅ | – | 🎯 core | Per-course student reviews with text, difficulty, helpfulness, self-reported grade, and ta… |
| node(id) on School (UF quality + departments) | POST | ✅ | – | ➕ enrich | departments[] gives a clean UF department taxonomy (id+name) usable to scope teacher searc… |
| Teacher.relatedFullTeachers / relatedTeachers | POST | ✅ | – | ➕ enrich | Suggest alternative instructors teaching similar/same UF courses (same department) — usefu… |
| teacherTags (global tag catalog) | POST | ✅ | – | ➕ enrich | Reference dictionary of all possible tag names to normalize/parse the '--'-delimited ratin… |
| spammedTeachers / user / oauthURL (root queries) | POST | ⚠️ | 🔒 | · none | None — these are account/moderation surfaces, not course/instructor data. Listed for compl… |
...
| Endpoint | M | Live | Auth | Rel | Use |
|---|---|---|---|---|---|
| Directory People Search (Faculty/Staff, public) | POST (also GET w/ query params) | ✅ | – | 🎯 core | Resolve SOC instructor names -> official ufl.edu email + faculty/staff flag; the canonical… |
| Directory Individual Detail (dept/title/affiliation) | GET | ✅ | – | 🎯 core | Get an instructor's home department + faculty-vs-staff classification to disambiguate whic… |
| Directory People Search (auth=true, full incl. students) | POST | 🔒 | 🔒 | · none | Do NOT use — student data behind SSO; auth=false already covers all instructors (faculty/s… |
| phonebook.ufl.edu (legacy host) | GET | ✅ | – | 🏛 heritage | Legacy alias — always hit directory.ufl.edu directly; phonebook just redirects. |
| GatorEvals Public Results page (Tableau embed shell) | GET | ✅ | – | ➕ enrich | Discovery anchor — tells you the exact Tableau Public workbook holding official UF eval sc… |
| Tableau Public Workbook Metadata API (GatorEvals) | GET | ✅ | – | ➕ enrich | Cheap freshness/version check (lastUpdateDate) and confirms the hidden raw-data sheet 'Gat… |
| GatorEvals raw data sheet (Tableau bootstrapSession) | GET view then POST bootstrap | ✅ | – | ➕ enrich | OFFICIAL UF course-evaluation scores per course/instructor/term — the institutional comple… |
...
**Host:** `libcal.uflib.ufl.edu (LibCal vanity for uflib.libcal.com) + ufl-flvc.primo.exlibrisgroup.com (Primo VE) + guides.uflib.ufl.edu / lgapi-us.libapps.com (LibGuides) + uflib.ufl.edu (portal)` — 14 endpoints, 11 live
...
| Endpoint | M | Live | Auth | Rel | Use |
|---|---|---|---|---|---|
| LibCal Hours Today (open, unauthenticated) | GET | ✅ | – | ➕ enrich | Real-time 'is the library open right now / today' for a where-to-study layer. 17 UF librar… |
| LibCal Hours Grid (multi-week) | GET | ✅ | – | ➕ enrich | Weekly study-hours planner + holiday closure detection (note field surfaces 'Memorial Day'… |
| LibCal Hours by Date | GET | ✅ | – | ➕ enrich | Hours for an arbitrary future date — 'will Marston be open the day my exam ends'. Lighter … |
| LibCal Hours Grid Widget (alias) | GET | ✅ | – | ➕ enrich | Backup/alias of the grid endpoint; use api_hours_grid.php as primary. |
| LibCal Spaces Availability Grid (study-room booking grid) | POST | ✅ | – | ➕ enrich | Live study-room availability for the where-to-study layer. Recipe: scrape gid from the /sp… |
| LibCal Calendar Events List (ajax) | GET | ✅ | – | ➕ enrich | Library workshops / research-skills events feed (low planner value). De-prioritize; the ca… |
| LibCal Equipment Availability Nextdate | GET | ✅ | – | · none | Equipment-loan availability. Not relevant to degree planning; skip. |
| LibCal v1.1 REST (documented Springshare API) — OAuth WALLED | GET | 🔒 | 🔒 | ➕ enrich | The richer official API (full space inventory, booking objects, calendar metadata) but nee… |
| Primo VE Public Search (primaws/pnxs) — UNAUTH catalog disco… | GET | ✅ | – | ➕ enrich | Live, fully-unauthenticated UF catalog discovery. For a course like CHM2210, query the tex… |
| Primo VE Public Configuration REST | GET | ✅ | – | ➕ enrich | Read once to discover valid tab/scope/query-field values for the pnxs search (the scopes-c… |
| Ex Libris Hosted Primo REST (api-na) — APIKEY WALLED | GET | 🔒 | 🔒 | ➕ enrich | The 'official' Primo API but requires an apikey. SKIP IT — the unauthenticated primaws/pnx… |
| LibGuides v1.2 API (lgapi-us) — APIKEY WALLED | GET | 🔒 | 🔒 | ➕ enrich | Subject research guides + databases-A-Z. Course-relevant ('research guide for ENC1101') bu… |
| LibGuides Public Site (guides.uflib.ufl.edu) — scrapable HTM… | GET | ✅ | – | ➕ enrich | If course/subject research guides are wanted without an API key, scrape the public HTML (S… |
| UF Libraries main site (uflib.ufl.edu) — hours/find portal | GET | ✅ | – | · none | Discovery hub only — gave us the Primo vid and the LibCal vanity host. Not a data API itse… |
...
**Host:** `campusmap.ufl.edu (+ stars.facilities.ufl.edu, calendar.ufl.edu, passio3.com, recsports.ufl.edu, goboardapi.azurewebsites.net)` — 13 endpoints, 12 live
...
| Endpoint | M | Live | Auth | Rel | Use |
|---|---|---|---|---|---|
| CampusMap searchBldg (building + room directory w/ lat-lon) | GET | ✅ | – | 🎯 core | THIS is the gold for 'time between back-to-back classes'. Map each section's building code… |
| CampusMap layer assets (60+ GeoJSON FeatureCollections) | GET | ✅ | – | ➕ enrich | Enrichment layer for a campus-aware planner: place student amenities (libraries, computer_… |
| CampusMap sidenav layer manifest | GET | ✅ | – | ➕ enrich | Self-describing manifest — fetch this first to discover every available campus layer + its… |
| CampusMap boundaries / parking_polys (large GeoJSON) | GET | ✅ | – | 🏛 heritage | Map-rendering polish only (draw campus footprint / parking zones). Not planner-critical. p… |
| STARS facilities public building photo | GET | ✅ | – | ➕ enrich | Show a real photo of each class building in the schedule/planner UI keyed by BLDG code. Pa… |
| CampusMap legacy cmapjson static files (DEAD) | GET | 🔒 | – | 🏛 heritage | DO NOT USE. The 2022 uf_api cmapjson endpoints are permanently 403'd. Their data lives on … |
| UF Events — LiveWhale Calendar JSON feed | GET | ✅ | – | ➕ enrich | Surface UF events alongside the academic calendar (deadlines, athletics, group events) by … |
| UF Events — LiveWhale iCal feed | GET | ✅ | – | ➕ enrich | Drop-in iCal subscription for users who want UF events in their own calendar app; or parse… |
| UF Transit (Passio GO) — stops + routes | POST | ✅ | – | ➕ enrich | 67 geocoded UF shuttle stops on 3 named routes (Central/North-South/East-West). Combine wi… |
| UF Transit (Passio GO) — live vehicle positions | POST | ✅ | – | 🏛 heritage | Real-time bus AVL. Low planner value (only useful day-of, and empty during breaks/summer).… |
| Passio GO system directory (getSystems) | GET | ✅ | – | 🏛 heritage | Discovery only — confirms UF system id 3826. Hardcode 3826 in the SDK; no need to re-query… |
| RecSports gym occupancy (Connect2Concepts / goboardapi) | GET | ✅ | – | ➕ enrich | Live real-time gym/pool crowding (% capacity) for SWRC, Southwest, Lake Wauburg, etc. Nice… |
| RecSports live gym cameras | GET | ✅ | – | 🏛 heritage | Live gym webcams (8 feeds). Pure student-life flavor — pair with the occupancy JSON for a … |
...
| Endpoint | M | Live | Auth | Rel | Use |
|---|---|---|---|---|---|
| SOC schedule (course/section list) | GET | ✅ | – | 🎯 core | Primary source for sections/terms, course descriptions, prerequisites (free-text edges to … |
| SOC filters (dictionaries) | GET | ✅ | – | 🎯 core | Dept-code(8-digit)->name dictionary, full term-code list (history back to Fall 2018), prog… |
| response-cache/soc (+/filters +/schedule) | GET | ✅ | – | · none | Server-side cache-warm / session-prime only. Returns no data. Do not use. |
| SimpleSyllabus document (UF tenant) | GET | 🔒 | 🔒 | ➕ enrich | Use section.simpleSyllabusParams as structured metadata (which term/subject/catalog/sectio… |
| myschedule course-search (authenticated SOC twin) | GET | 🔒 | 🔒 | · none | Authenticated personalization variant of the public schedule endpoint. Out of scope (login… |
...
**Host:** `catalog.ufl.edu (+ one.ufl.edu, ir.aa.ufl.edu, data-apps.ir.aa.ufl.edu, uf.maps.arcgis.com, campusmap.ufl.edu)` — 15 endpoints, 11 live
...
| Endpoint | M | Live | Auth | Rel | Use |
|---|---|---|---|---|---|
| CourseLeaf ribbit getcourse (course catalog + prereq edges) … | GET | ✅ | – | 🎯 core | Complete UF course catalog: descriptions, credits, grading scheme, AND prerequisite chain … |
...
| Catalog program/degree page (plangrid degree requirements + … | GET | ✅ | – | 🎯 core | De-facto degree-requirements API: required courses per term, total credits, Critical Track… |
| Catalog programs/majors index (canonical major codes + colle… | GET | ✅ | – | 🎯 core | Canonical UF undergraduate program list + codes (COLLEGE/MAJOR_DEGREE) — the major picker … |
| OneUF apix soc filters (term/category/level enumerator) | GET | ✅ | – | ➕ enrich | Bootstrap valid category/level/term values before calling apix/soc/schedule (sections). Co… |
| OneUF apix soc schedule (sections + description) — confirm-l… | GET | ✅ | – | 🎯 core | Sections/terms/seats/instructors per course (Gradvisr already integrates soc; confirmed st… |
| IR data-apps public file tree (PBF/SUSF/accountability metho… | GET | ✅ | – | 🏛 heritage | Excess-hours surcharge + graduation/retention methodology PDFs are useful context for warn… |
| IR accountability metrics page (links to UF performance/CDS-… | GET | ✅ | – | 🏛 heritage | Index of UF accountability/performance docs; low direct planner value, useful as instituti… |
| UF Facts (IR) — embeds Tableau Public vizzes (enrollment/deg… | GET | ✅ | – | 🏛 heritage | Enrollment & degrees-conferred aggregates exist but only via embedded Tableau Public; not … |
| CourseLeaf ribbit getprogram (program XML) — DEAD code forma… | GET | ⚠️ | – | · none | Not usable — the program XML endpoint returns empty for all guessed codes; get degree requ… |
| IR dailyEnrollment app (live enrollment by course) — SSO WAL… | GET | 🔒 | 🔒 | · none | Would give live per-course enrollment but is behind UF SSO — out of bounds. Use apix/soc o… |
